...
class SwitchBackup:
    ...
    def setup_gui(self):
        ...
        style = ttk.Style()
        style.configure("Custom.TLabelframe.Label", font=("Roboto", 10, "bold"), foreground="#ffffff")
        style.configure("Custom.TLabel", font=("Roboto", 8), foreground="#e0e0e0")
        style.configure("Custom.TButton", font=("Roboto", 8), padding=5)
        style.configure("Custom.TCheckbutton", font=("Roboto", 8))
        style.configure("Custom.TEntry", font=("Roboto", 8))
        style.configure("Custom.Treeview", font=("Roboto", 8), rowheight=20)
        style.configure("Custom.Treeview.Heading", font=("Roboto", 9, "bold"))
        ...
        version_label = ttk.Label(right_column, text=f"Version: {self.VERSION}", font=("Roboto", 8, "italic"), bootstyle="light")
        version_label.pack(side="bottom", anchor="se", pady=10)

        csv_frame = ttk.LabelFrame(left_column, text="Switch Inventory", padding="10", bootstyle="primary", style="Custom.TLabelframe")
        csv_frame.pack(fill="x", pady=(0, 10))
        self.path_label = ttk.Label(csv_frame, text=self.csv_file, wraplength=300, style="Custom.TLabel")
        self.path_label.pack(pady=3)
        csv_button = ttk.Button(csv_frame, text="Select CSV", command=self.select_csv, bootstyle="primary", style="Custom.TButton")
        csv_button.pack(pady=3)

        ttk.Separator(left_column, orient="horizontal").pack(fill="x", pady=5)

        backup_loc_frame = ttk.LabelFrame(left_column, text="Backup Location", padding="10", bootstyle="primary", style="Custom.TLabelframe")
        backup_loc_frame.pack(fill="x", pady=10)
        self.backup_loc_label = ttk.Label(backup_loc_frame, text=self.base_dir if self.base_dir else "Not set", wraplength=300, style="Custom.TLabel")
        self.backup_loc_label.pack(pady=3)
        backup_dir_button = ttk.Button(backup_loc_frame, text="Select Dir", command=self.select_backup_dir, bootstyle="primary", style="Custom.TButton")
        backup_dir_button.pack(pady=3)

        ttk.Separator(left_column, orient="horizontal").pack(fill="x", pady=5)

        cred_frame = ttk.LabelFrame(left_column, text="REST API Credentials", padding="10", bootstyle="primary", style="Custom.TLabelframe")
        cred_frame.pack(fill="x", pady=10)
        cred_subframe = ttk.Frame(cred_frame)
        cred_subframe.pack(expand=True)
        ttk.Label(cred_subframe, text="Username:", style="Custom.TLabel").pack(side="left", padx=3)
        self.username_entry = ttk.Entry(cred_subframe, width=15, style="Custom.TEntry")
        self.username_entry.insert(0, self.default_username)
        self.username_entry.pack(side="left", pady=3)
        ttk.Label(cred_subframe, text="Password:", style="Custom.TLabel").pack(side="left", padx=3)
        self.password_entry = ttk.Entry(cred_subframe, show="*", width=15, style="Custom.TEntry")
        self.password_entry.insert(0, self.default_password)
        self.password_entry.pack(side="left", pady=3)
        cred_save_button = ttk.Button(cred_subframe, text="Save", command=self.save_credentials, bootstyle="success", style="Custom.TButton")
        cred_save_button.pack(pady=3)

        ttk.Separator(left_column, orient="horizontal").pack(fill="x", pady=5)

        sched_frame = ttk.LabelFrame(left_column, text="Schedule", padding="10", bootstyle="primary", style="Custom.TLabelframe")
        sched_frame.pack(fill="x", pady=10)
        freq_frame = ttk.Frame(sched_frame)
        freq_frame.pack(fill="x", pady=3)
        self.freq_var = tk.StringVar(value=self.schedule_frequency)
        ttk.Radiobutton(freq_frame, text="Daily", value="daily", variable=self.freq_var, command=self.update_schedule_details, style="Custom.TCheckbutton").pack(side="left", padx=2, expand=True)
        ttk.Radiobutton(freq_frame, text="Weekly", value="weekly", variable=self.freq_var, command=self.update_schedule_details, style="Custom.TCheckbutton").pack(side="left", padx=2, expand=True)
        ttk.Radiobutton(freq_frame, text="Custom", value="custom", variable=self.freq_var, command=self.update_schedule_details, style="Custom.TCheckbutton").pack(side="left", padx=2, expand=True)
        self.sched_details_frame = ttk.Frame(sched_frame)
        self.sched_details_frame.pack(fill="x", pady=3)
        self.update_schedule_details()
        self.schedule_toggle_var = tk.BooleanVar(value=self.schedule_enabled)
        sched_toggle = ttk.Checkbutton(sched_frame, text="Enable", variable=self.schedule_toggle_var, command=self.toggle_schedule, style="Custom.TCheckbutton")
        sched_toggle.pack(pady=3)

        ttk.Separator(left_column, orient="horizontal").pack(fill="x", pady=5)

        control_frame = ttk.LabelFrame(left_column, text="Manual Backup", padding="10", bootstyle="primary", style="Custom.TLabelframe")
        control_frame.pack(fill="x", pady=10)
        run_backup_button = ttk.Button(control_frame, text="Run Now", command=self.manual_backup, bootstyle="success", style="Custom.TButton")
        run_backup_button.pack(pady=3)
        ...
        git_frame = ttk.LabelFrame(right_column, text="Git Repository", padding="10", bootstyle="primary", style="Custom.TLabelframe")
        git_frame.pack(fill="x", pady=(0, 10))
        self.git_enabled_var = tk.BooleanVar(value=self.git_enabled)
        git_toggle = ttk.Checkbutton(git_frame, text="Enable", variable=self.git_enabled_var, command=self.toggle_git, style="Custom.TCheckbutton")
        git_toggle.pack(pady=3)
        ttk.Label(git_frame, text="Repo URL:", style="Custom.TLabel").pack(pady=1)
        self.git_repo_entry = ttk.Entry(git_frame, width=20, style="Custom.TEntry")
        self.git_repo_entry.insert(0, self.git_repo_url)
        self.git_repo_entry.pack(pady=1)
        ttk.Label(git_frame, text="Token:", style="Custom.TLabel").pack(pady=1)
        self.git_token_entry = ttk.Entry(git_frame, show="*", width=20, style="Custom.TEntry")
        self.git_token_entry.insert(0, self.git_token)
        self.git_token_entry.pack(pady=1)
        git_save_button = ttk.Button(git_frame, text="Save", command=self.save_git_settings, bootstyle="success", style="Custom.TButton")
        git_save_button.pack(pady=3)

        ttk.Separator(right_column, orient="horizontal").pack(fill="x", pady=5)

        wasabi_frame = ttk.LabelFrame(right_column, text="Wasabi Storage", padding="10", bootstyle="primary", style="Custom.TLabelframe")
        wasabi_frame.pack(fill="x", pady=(0, 10))
        self.wasabi_enabled_var = tk.BooleanVar(value=self.wasabi_enabled)
        wasabi_toggle = ttk.Checkbutton(wasabi_frame, text="Enable", variable=self.wasabi_enabled_var, command=self.toggle_wasabi, style="Custom.TCheckbutton")
        wasabi_toggle.pack(pady=3)
        ttk.Label(wasabi_frame, text="Access Key:", style="Custom.TLabel").pack(pady=1)
        self.wasabi_access_key_entry = ttk.Entry(wasabi_frame, width=20, style="Custom.TEntry")
        self.wasabi_access_key_entry.insert(0, self.wasabi_access_key)
        self.wasabi_access_key_entry.pack(pady=1)
        ttk.Label(wasabi_frame, text="Secret Key:", style="Custom.TLabel").pack(pady=1)
        self.wasabi_secret_key_entry = ttk.Entry(wasabi_frame, show="*", width=20, style="Custom.TEntry")
        self.wasabi_secret_key_entry.insert(0, self.wasabi_secret_key)
        self.wasabi_secret_key_entry.pack(pady=1)
        ttk.Label(wasabi_frame, text="Bucket:", style="Custom.TLabel").pack(pady=1)
        self.wasabi_bucket_entry = ttk.Entry(wasabi_frame, width=20, style="Custom.TEntry")
        self.wasabi_bucket_entry.insert(0, self.wasabi_bucket)
        self.wasabi_bucket_entry.pack(pady=1)
        ttk.Label(wasabi_frame, text="Region:", style="Custom.TLabel").pack(pady=1)
        self.wasabi_region_entry = ttk.Entry(wasabi_frame, width=20, style="Custom.TEntry")
        self.wasabi_region_entry.insert(0, self.wasabi_region)
        self.wasabi_region_entry.pack(pady=1)
        wasabi_save_button = ttk.Button(wasabi_frame, text="Save", command=self.save_wasabi_settings, bootstyle="success", style="Custom.TButton")
        wasabi_save_button.pack(pady=3)

        status_frame = ttk.Frame(notebook, padding="10")
        notebook.add(status_frame, text="Status")
        self.status_tree = ttk.Treeview(status_frame, columns=("Name", "IP", "Last Backup", "Status", "Git Status", "Wasabi Status"), show="headings", style="Custom.Treeview")
        ...
        refresh_button = ttk.Button(status_frame, text="Refresh", command=self.refresh_status, style="Custom.TButton")
    ...
    def update_schedule_details(self):
        ...
        if self.freq_var.get() == "daily":
            ...
            ttk.Label(times_frame, text="Times (HH:MM):", style="Custom.TLabel").pack(side="left", padx=2)
            self.daily_times_entry = ttk.Entry(times_frame, width=15, style="Custom.TEntry")
            self.daily_times_entry.insert(0, ", ".join(self.schedule_times))
            self.daily_times_entry.pack(side="left", padx=2)
            ttk.Button(times_frame, text="Set", command=self.update_schedule, style="Custom.TButton").pack(side="left", padx=5)
        else:
            details_frame = ttk.Frame(self.sched_details_frame)
            details_frame.pack(expand=True)
            ttk.Label(details_frame, text="Day:", style="Custom.TLabel").pack(side="left", padx=2)
            if self.freq_var.get() == "weekly":
                ...
                ttk.Radiobutton(details_frame, text=self.schedule_day[:3], value=self.schedule_day, variable=self.custom_day_var, style="Custom.TCheckbutton").pack(side="left", padx=2)
            ttk.Label(details_frame, text="Time:", style="Custom.TLabel").pack(side="left", padx=2)
            ...
            ttk.Button(details_frame, text="Set", command=self.update_schedule, style="Custom.TButton").pack(side="left", padx=5)
